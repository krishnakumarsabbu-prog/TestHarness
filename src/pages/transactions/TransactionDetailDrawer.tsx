...
const logLevelConfig = {
  INFO: { text: 'text-primary-600', bg: 'bg-primary-50', border: 'border-primary-100' },
  DEBUG: { text: 'text-warm-600', bg: 'bg-surface-100', border: 'border-surface-200' },
...
function DetailRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-start justify-between gap-4 py-2.5 border-b border-surface-100 last:border-0">
      <span className="text-xs font-semibold text-warm-400 uppercase tracking-wider whitespace-nowrap">{label}</span>
      <span className="text-sm text-warm-700 text-right font-medium">{value}</span>
    </div>
  )
}
...
function TransactionDetailDrawer({ transaction, onClose }: TransactionDetailDrawerProps) {
  ...
  return (
    <>
      ...
      <div
        className="fixed right-0 top-0 h-full w-full max-w-xl bg-ivory shadow-soft-lg z-50 flex flex-col"
      ...
      >
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-surface-200 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-warm-900">{transaction.alertName}</p>
              <p className="text-xs text-warm-400 font-mono">{transaction.messageId}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="p-1.5 rounded-lg text-warm-400 hover:text-warm-700 hover:bg-surface-100 transition-colors duration-150"
            >
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="px-6 py-5 space-y-6">
            <section>
              <h3 className="text-xs font-bold text-warm-400 uppercase tracking-widest mb-3">Lifecycle</h3>
            ...
            <section>
              <h3 className="text-xs font-bold text-warm-400 uppercase tracking-widest mb-3">Payload</h3>
            ...
            <section>
              <h3 className="text-xs font-bold text-warm-400 uppercase tracking-widest mb-3">Execution Logs</h3>
